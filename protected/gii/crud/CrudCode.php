...
class CrudCode extends CCodeModel
{
	...
	{
		return strtolower($this->_modelClass);
	}	
	...
	{
		return "CHtml::activeLabelEx(\$".strtolower($modelClass).",'{$column->name}')";
	}

	public function generateInputField($modelClass,$column)
	{
		if($column->type==='boolean')
			return "CHtml::activeCheckBox(\$".strtolower($modelClass).",'{$column->name}')";
		elseif(stripos($column->dbType,'text')!==false)
			return "CHtml::activeTextArea(\$".strtolower($modelClass).",'{$column->name}',array('rows'=>6, 'cols'=>50))";
		else
		{
			...
			if($column->type!=='string' || $column->size===null)
				return "CHtml::{$inputField}(\$".strtolower($modelClass).",'{$column->name}')";
			else
			{
				if(($size=$maxLength=$column->size)>60)
					$size=60;
				return "CHtml::{$inputField}(\$".strtolower($modelClass).",'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength))";
			}
		}
	}
	...
	{
		return "\$form->labelEx(\$".strtolower($modelClass).",'{$column->name}')";
	}

	public function generateActiveField($modelClass,$column)
	{
		if($column->type==='boolean')
			return "\$form->checkBox(\$".strtolower($modelClass).",'{$column->name}')";
		elseif(stripos($column->dbType,'text')!==false)
			return "\$form->textArea(\$".strtolower($modelClass).",'{$column->name}',array('rows'=>6, 'cols'=>50))";
		else
		{
			if(preg_match('/^(password|pass|passwd|passcode)$/i',$column->name))
			...
			if($column->type!=='string' || $column->size===null)
				return "\$form->{$inputField}(\$".strtolower($modelClass).",'{$column->name}')";
			else
			{
				if(($size=$maxLength=$column->size)>60)
					$size=60;
				return "\$form->{$inputField}(\$".strtolower($modelClass).",'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength))";
			}
		}
	}
